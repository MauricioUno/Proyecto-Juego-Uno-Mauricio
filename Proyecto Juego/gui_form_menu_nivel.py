...
class FormMenuNiveles(Form):
    def __init__(self,name,master_surface,x=0,y=0,w=ANCHO_VENTANA,h=ALTO_VENTANA,color_background= None, imagen_background = None, color_border= None,active=False):
        ...
        self.boton1 = Button(master=self,x=20,y=20,w=750,h=180,image_background=PATH_RECURSOS + r"\background\Forest0.png",on_click=self.start_level,on_click_param=1)
        self.boton2 = Button(master=self,x=20,y=220,w=750,h=180,image_background=PATH_RECURSOS + r"\background\Snow3.png",on_click=self.start_level,on_click_param=2)
        self.boton3 = Button(master=self,x=20,y=420,w=750,h=180,image_background=PATH_RECURSOS + r"\background\Cave0.png",on_click=self.start_level,on_click_param=3)
        self.boton4 = Button(master=self,x=20, y=670, w=120,h =50,image_background=None, on_click=self.retroceder, on_click_param="main",text="atras", font_size= 50, font_color=COLOR_TEXTO_MENU) 
    ...
    def data_levels(self, nivel, boton):
        if not obtener_estado_nivel(nivel)[0]:
            boton.active = False
